...
module.exports = {
	gobble: function (xml) {
		...
		while (firstGreaterThanIndex > -1 && subsequentLessThanIndex > -1 && firstGreaterThanIndex < subsequentLessThanIndex) {
			...
			outXml.push(new Buffer(xml.substring(0, subsequentLessThanIndex - trimmedGap)));
		...
		function getRight(xml, endLeft) {
			return xml.substring(endLeft).indexOf('<') + endLeft;
		}

		outXml.push(new Buffer(xml));
		return Buffer.concat(outXml).toString();
	}
}
